        ...
        <v-col>
          ...
          >
            <div class="col-start-2 flex flex-col items-center">
              <h2 class="text-center text-xl">{{ headline }}</h2>
              <h3 class="text-center">{{ classificationResultLabel }}</h3>
            </div>
...
const headline = computed(() => {
  let suffix = results.value && results.value.length > 0 ? "" : "...";
...
const classificationResultLabel = computed(() => {
  ...
  if (topResult) {
    return `${(topResult.confidence * 100).toFixed(2)}% ${topResult.label}`;
  }
});
