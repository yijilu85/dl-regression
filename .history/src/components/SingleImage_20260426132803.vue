...
const data = {
  ...
  scales: {
    y: {
      ticks: {
        callback: (value) => `${(value * 100).toFixed(0)}%`, // or just value * 100
      },
    },
  },
};
...
const bgColor = computed(() => {
  if (results.value && results.value.length > 0) {
    return props.correct ? "bg-green-80" : "bg-red-80";
  }
  return "bg-gray-100";
});
...
function gotResult(res: Result[]) {
  ...
  const sorted = [...res].sort((a, b) => b.confidence - a.confidence);
  results.value = sorted.map(
    (r) =>
      ({
        label: r.label,
