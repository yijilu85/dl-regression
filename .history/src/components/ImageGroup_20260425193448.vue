<template>
  ...
  <div class="mt-2">
    ...
    />
    <div
      v-for="(item, index) in uploadedFilePreviews"
      :key="item.url"
      class="preview-item mt-4 mb-4 fade-item"
      @remove="handleRemoveImage(index, uploadedFilePreviews)"
      :class="{ removing: removingIndex === index }"
    >
      <SingleImage :imgSrc="item.url" :correct="true" />
    </div>
    <FileUpload
...
const handleRemoveImage = (index: number, list: string[]) => {
