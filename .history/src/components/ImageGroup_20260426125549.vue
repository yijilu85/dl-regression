<template>
  <div class="mt-2">
    <div class="flex items-center justify-between pt-4 gap-4 ml-4">
      <h2 class="text-xl fold-bold">{{ groupData.name }}</h2>
      <v-btn prepend-icon="mdi-refresh" variant="plain" @click="resetGroup">
...
const initialImages = [...props.groupData.images];
const visibleImages = ref<string[]>([...initialImages]);
...
const resetGroup = () => {
  console.log(
    "before Resetting group:",
    props.groupData.name,
    visibleImages.value,
  );
  visibleImages.value = [...initialImages];
  console.log(
    "after Resetting group:",
    props.groupData.name,
    visibleImages.value,
  );
